...
output "jumphost_shared_public_key" {
...
output "jumphost_shared_private_key" {
...
output "tgw_jumphost_vpc_id" {
  description = "ID of the VPC containing the TGW jumphost"
  value       = var.create_tgw_jumphost ? local.tgw_vpc_id : "TGW jumphost not created"
}

output "tgw_jumphost_vpc_name" {
  description = "Name of the VPC containing the TGW jumphost"
  value       = var.create_tgw_jumphost ? local.tgw_vpc_name : "TGW jumphost not created"
}

output "tgw_jumphost_id" {
  description = "Instance ID of the TGW jumphost"
  value       = var.create_tgw_jumphost ? ibm_is_instance.tgw_jumphost[0].id : "TGW jumphost not created"
}

output "tgw_jumphost_private_ip" {
  description = "Private IP address of the TGW jumphost"
  value       = var.create_tgw_jumphost ? ibm_is_instance.tgw_jumphost[0].primary_network_interface[0].primary_ip[0].address : "TGW jumphost not created"
}

output "tgw_jumphost_public_ip" {
  description = "Floating (public) IP address of the TGW jumphost"
  value       = var.create_tgw_jumphost ? ibm_is_floating_ip.tgw_jumphost_fip[0].address : "TGW jumphost not created"
}

output "tgw_jumphost_ssh_command" {
  description = "SSH command to connect to the TGW jumphost"
  value = var.create_tgw_jumphost ? (
    var.ssh_key_name != "" ?
    "ssh -i ${var.ssh_key_name} ubuntu@${ibm_is_floating_ip.tgw_jumphost_fip[0].address}" :
    "ssh ubuntu@${ibm_is_floating_ip.tgw_jumphost_fip[0].address}"
  ) : "TGW jumphost not created"
}

output "tgw_jumphost_zone" {
  description = "Availability zone where the TGW jumphost was placed"
  value       = var.create_tgw_jumphost ? local.tgw_jumphost_zone : "TGW jumphost not created"
}

output "tgw_jumphost_profile_used" {
  description = "Instance profile selected for the TGW jumphost"
  value       = var.create_tgw_jumphost ? local.tgw_jumphost_profile : "TGW jumphost not created"
}
...
output "transit_gateway_connection_id" {
  ...
  value       = (var.create_tgw_jumphost && var.transit_gateway_name != "") ? ibm_tg_connection.tgw_vpc_connection[0].connection_id : "TGW connection not created"
}
...
output "cluster_jumphost_private_ips" {
...
output "cluster_jumphost_public_ips" {
...
output "cluster_jumphost_ssh_commands" {
  ...
  value = {
    ...
    zone => (var.ssh_key_name != "" ? "ssh -i ${var.ssh_key_name} ubuntu@${fip.address}" : "ssh ubuntu@${fip.address}")
  }
}
